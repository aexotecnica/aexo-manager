...
<div class="container">
	<div class="panel panel-midnightblue">
		<div class="row">
							...
							<tbody>
								...
								if ($ordenes != NULL)
									foreach ($ordenes as $val){?>	
										<tr class="idOrdenPedido gradeX">
											<td style="display:none"><?= $val->idOrdenPedido?></td>
											<td><?= $val->nroPedido?></td>
			...
			<input type="hidden" id="idOrdenPedido" name="idOrdenPedido"></input>
		</div>
	</div>
</div>
</div>
...
$( document ).ready(function() {
	...
	$("#btnModificar").click(function () {
		if ($('#idOrdenPedido').val() != ''){
			$("#formBody").attr("action", "<?= base_url() ?>index.php/ordenPedido/modificar");
			$("#formBody").submit();
		}else {
    ...
    $('#dtOrdenes tbody').on( 'click', 'tr', function () {
        $("#idOrdenPedido").val($(this).children("td:eq(0)").text());
    } );
